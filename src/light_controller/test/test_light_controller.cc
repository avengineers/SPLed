...
TEST(light_controller, test_light_on_and_off)
{
    ...
    EXPECT_CALL(mymock, RteSetLightValue(RGBColorEq(onColor))).Times(1); // Expect that the light value changes to ON color.
    lightController();

    // Power remains ON, but the light should not change (no redundant updates).
    EXPECT_CALL(mymock, RteGetPowerState()).WillOnce(Return(POWER_STATE_ON));
    EXPECT_CALL(mymock, RteSetLightValue(_)).Times(0); // Expect that the light value doesn't change.
    lightController();
    ...
    EXPECT_CALL(mymock, RteSetLightValue(RGBColorEq(offColor))).Times(1); // Expect that the light value changes to OFF color.
    lightController();

    // Power remains OFF, but the light should not change (no redundant updates).
    EXPECT_CALL(mymock, RteGetPowerState()).WillOnce(Return(POWER_STATE_OFF));
    EXPECT_CALL(mymock, RteSetLightValue(_)).Times(0); // Expect that the light value doesn't change.
    lightController();
}
